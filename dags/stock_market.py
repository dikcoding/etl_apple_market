...
# Difine DAG harian untuk pipeline stock market
@dag(
...
def stock_market():
    ...
    def is_api_available() -> PokeReturnValue:
        ...
        # Get API configuration from Airflow Connection 
        api = BaseHook.get_connection('stock_api')
        url = f"{api.host}{api.extra_dejson["endpoint"]}"
        print(url)

        # Get API https://query1.finance.yahoo.com/v8/finance/chart/nvda?metrics=high?&interval=1d&range=1y
        response = requests.get(url, headers=api.extra_dejson['headers'])

        # Determine whether the API is available
    ...
    format_price = DockerOperator(
        task_id='format_price',
    ...
    is_api_available() >> get_stock_prices >> store_prices >> format_price >> get_formatted_csv >> load_to_dw
